...
export default function POSPage() {
  ...
  const pageStyle: React.CSSProperties = { display: 'flex', flexDirection: isMobile ? 'column' : 'row', height: isMobile ? 'auto' : '100%', minHeight: isMobile ? '100%' : undefined, backgroundColor: C.bg, fontFamily: '-apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, sans-serif', overflow: isMobile ? 'auto' : 'hidden', position: 'relative' }
  const leftPanelStyle: React.CSSProperties = { flex: isMobile ? '1 1 auto' : rv('1 1 auto', '0 0 55%', '0 0 60%'), display: 'flex', flexDirection: 'column', padding: rv(12, 16, 20), overflow: isMobile ? 'visible' : 'hidden', minHeight: isMobile ? 'auto' : undefined }
  ...
  const gridContainerStyle: React.CSSProperties = { flex: isMobile ? 'none' : 1, overflowY: isMobile ? 'visible' : 'auto', paddingRight: 4, paddingBottom: isMobile ? 70 : 0 }
        ...
        {currentStore?.id && (
          <div style={{
            ...
            <button
              ...
              onClick={() => {
                const url = `${window.location.origin}/catalog?store=${currentStore.id}`
                const msg = `${currentStore.name || 'Notre boutique'}\n${(t as any).pos?.catalogMsg || 'Découvrez nos produits et services en ligne !'}\n\n${url}`
            ...
            >
              ...
              {(t as any).pos?.sendCatalog || 'Envoyer le catalogue'}
            </button>
            <button
              style={{
                display: 'flex',
              ...
              onClick={async () => {
                const url = `${window.location.origin}/catalog?store=${currentStore.id}`
                if (navigator.share) {
                  navigator.share({ title: currentStore.name || '', text: (t as any).pos?.catalogMsg || 'Découvrez nos produits et services en ligne !', url }).catch(() => {})
                } else {
            ...
            >
              <Share2 size={15} />
              {(t as any).pos?.shareCatalog || 'Partager'}
            </button>
          </div>
        )}
